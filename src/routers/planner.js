...
//import planner json
router.get("/insert-planners-json", async (req, res) => {
  try {
    let data = fs.readFileSync("./src/json/planner.json");
    data = JSON.parse(data);
    Planner.insertMany(data);
    res.status(200).send({ 
			success: true, 
			response: { code: 200, message: "successfully imported Planner json data to mongoDB" }
		});
  } catch (error) {
    console.error(error);
...
router.get("/get-planners", async (req, res) => {
  try {
    const planners = await Planner.find({ isDeleted: false }, null, {
      sort: {
      ...
      .populate("Recipes")
      .exec();
    res.status(200).send({ success: true, response: { code: 200, planners } });
  } catch (e) {
    res
      .status(400)
      .send({ success: false, response: { code: 400, message: e.name } });
